...
class AllNewsView extends StatelessWidget {
  String? type;
  AllNewsView({super.key, this.type});
...
class FilterBookList extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final AllNewsControllers controller = Get.put(AllNewsControllers());
    final category = controller.categories[index];
    var categoryId = category.id;
    print("Received Index and category id is $index and $categoryId");
    return GetBuilder<AllNewsControllers>(
        builder: (controller) {
          return Visibility(
            visible: controller.isSelected.value,
            child: ListView.builder(
              ...
              itemBuilder: (context, index){
                ...
                final books = controller.categoryWiseNews[categoryId] ?? [];
                return Container(
                  child: Column(
                    children: [
                      ...
                      Container(
                        ...
                        child: FutureBuilder(
                          future: controller.getCategoryWiseNews(5, "in"),
                          builder: (context, snapshot) {
                            if (snapshot.connectionState == ConnectionState.waiting) {
                              return const Center(child: CircularProgressIndicator());
                            } else if (snapshot.hasError) {
                                  ...
                                  child: books.isNotEmpty
                                    ...
                                    itemBuilder: (context, subIndex) {
                                      ...
                                      return GestureDetector(
                                        onTap: (){
                                          print("Click ${book.id}");
                                          // Get.to(() => ProductDetailView(id: book.id));
                                        },
                                        child: Container(
                                          width: 160,
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              SizedBox(height: 5,),
                                              BookNameText(text: book.title.toString(),),
                                              SizedBox(height: 5,),
...
class NewReleaseNews extends StatelessWidget {
  final String language;
  final String type;
  const NewReleaseNews({super.key, required this.language, required this.type});

  @override
  Widget build(BuildContext context) {
    final controller = Get.find<AllNewsControllers>();
    return Obx(() => controller.isNewReleaseLoading.value
        ? DetailsPageShimmer()
        : Container(
            height: 275,
            margin: EdgeInsets.only(left: 10, right: 10,),
            child: Column(
              children: [
                ...
                Obx(()=> controller.isNewReleaseLoading.value ? Container() :
                  SizedBox(
                    height: 225,
                    child: FutureBuilder(
                      ...
                      builder: (context, snapshot) {
                        return ListView.builder(
                          ...
                          itemBuilder: (context, index){
                            ...
                            return Container(
                              width: 140,
                              margin: EdgeInsets.only(left: 10.w, right: 10.w, top: 8.h, bottom: 8.h,),
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(16),
                                color: Colors.grey.shade200,
                              ),
